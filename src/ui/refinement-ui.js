...
(()=>{
  ...
  $('exportDesignPNG').onclick=async()=>{await flushUpdate();const c=document.createElement('canvas');c.width=3000;c.height=2200;generator.draw(c.getContext('2d'),currentResult(),true,window.printStrokeScale||1);c.toBlob(async blob=>{if(blob)download(await PlateCodec.pngDpi(blob,3000,Number($('paperMM').value)),'刻间-刻线-3000.png');});};
  $('exportDesignSVG').onclick=async()=>{await flushUpdate();const mm=Number($('paperMM').value),scale=window.printStrokeScale||1;const paths=currentResult().paths.map(p=>`<path fill="none" stroke="#292d27" stroke-width="${(p.width*scale).toFixed(3)}" stroke-linecap="round" stroke-linejoin="round" d="M${p.points.map(q=>q.map(v=>v.toFixed(3)).join(',')).join('L')}"/>`).join('');download(new Blob([`<svg xmlns="http://www.w3.org/2000/svg" width="${mm}mm" height="${mm*660/900}mm" viewBox="0 0 900 660">${paths}</svg>`],{type:'image/svg+xml'}),'刻间-矢量刻线.svg');};
  window.refinement={
    recipe:settings,
