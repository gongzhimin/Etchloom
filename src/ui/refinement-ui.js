...
(()=>{
  ...
  function settings(fresh=false){
    const p={version:1};for(const key of Object.keys(PhotoPro.defaults))if(key!=='style'&&key!=='curvature')p[key]=Number($('pro-'+key)?.value||PhotoPro.defaults[key]);
    p.style=$('photoStyle')?.value||'engraving';
    p.curvature=75;
  ...
  window.refinement={
    ...
    sync(r){controls.hidden=r.mode!=='photo';compare.hidden=r.mode!=='photo';sourceStage.hidden=r.mode!=='photo'||!$('showComparison').checked;pair.classList.toggle('split',!sourceStage.hidden);
      if(r.mode==='photo'){for(const key of Object.keys(PhotoPro.defaults)){const v=r.pro?.[key]??PhotoPro.defaults[key];if(key==='style'&&$('photoStyle'))$('photoStyle').value=v;else if($('pro-'+key)){$('pro-'+key).value=v;if($('pro-'+key+'-value'))$('pro-'+key+'-value').value=v;}}syncSimple();$('cropNote').textContent=original?'原图仅保留在当前会话，可重新裁切；刷新后需重新上传。':'恢复的是分析图，重新裁切需要上传原图。';}
